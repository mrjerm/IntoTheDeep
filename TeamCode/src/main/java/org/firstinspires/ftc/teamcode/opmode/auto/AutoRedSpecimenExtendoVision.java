...
@Disabled
@Autonomous
public class AutoRedSpecimenExtendoVision extends OpMode {
...
//    private Path scorePreload, park;
    private PathChain scorePreload, intakeSpecimen1, intakeSpecimen2, intakeSpecimen3, wallSpecimen1, scoreSpecimen1, wallSpecimen2, scoreSpecimen2, wallSpecimen3, scoreSpecimen3, wallSpecimen4, scoreSpecimen4, park;
    ...
    public void buildPaths(){
        ...
        wallSpecimen1 = follower.pathBuilder()
                ...
                .setLinearHeadingInterpolation(redSpecimen_spikeRightPose.getHeading(), Math.toRadians(30), 0.3)
                .setLinearHeadingInterpolation(Math.toRadians(30), redSpecimen_wallSpecimenPose.getHeading())
                .build();

        scoreSpecimen1 = follower.pathBuilder()
                .addPath(new BezierLine(new Point(redSpecimen_wallSpecimenPose), new Point(redSpecimen_score1Pose)))
        ...
        switch (pathState){
            ...
            case 3:
                ...
                if (Math.abs(follower.getPose().getX() - redSpecimen_spikeMiddlePose.getX()) < positionThreshold && Math.abs(follower.getPose().getY() - redSpecimen_spikeMiddlePose.getY()) < positionThreshold){
                    ...
                    if (pathTimer.getElapsedTime() - pathTimeStamp > 2300){
                        // hold endpoint while scoring sample
                        setExtendo(extendoMax);
                        setIntake(0.6, intakeFlexorHover, intakeElbowHover - 0.04);
                        follower.followPath(intakeSpecimen3);
                        setPathState(4);
                        debounce = true;
                        timerActive = false;
                    }
            ...
            case 4:
                if (Math.abs(follower.getPose().getX() - redSpecimen_spikeRightPose.getX()) < positionThreshold && Math.abs(follower.getPose().getY() - redSpecimen_spikeRightPose.getY()) < positionThreshold){
                    ...
                    if (pathTimer.getElapsedTime() - pathTimeStamp > 1300){
                        // hold endpoint while scoring sample
            ...
            case 5:
                ...
                if (pathTimer.getElapsedTime() >= 300 && pathTimer.getElapsedTime() < 500){
                    intakeGrabber.setPosition(intakeGrabberOpen);
                }
            ...
            case 6:
                ...
                if (Math.abs(follower.getPose().getX() - redSpecimen_score1Pose.getX()) < positionThreshold && Math.abs(follower.getPose().getY() - redSpecimen_score1Pose.getY()) < positionThreshold){
                    setOuttakeGrabber(true);
                    // start timer
                    if (!timerActive){
                        pathTimeStamp = pathTimer.getElapsedTime();
                    ...
                    if (pathTimer.getElapsedTime() - pathTimeStamp > 300){
                        // hold endpoint while scoring sample
//                        follower.followPath(wallSpecimen2);
                        setPathState(7);
                        debounce = true;
                        timerActive = false;
                    }
    ...
    public void loop() {
        ...
        if (result != null && result.isValid()) {
            ...
            pose = new Pose(-1 * botpose.getPosition().y * 39.3700787 + 72, botpose.getPosition().x * 39.3700787 + 72, botpose.getOrientation().getYaw(AngleUnit.RADIANS));
            telemetry.addLine("detected pose: " + pose);
        }
