...
@Autonomous(name = "Autonomous Red Specimen 1+0+3")
public class AutoRedSpecimenPreloadOnlyWIP extends OpMode {
...
//    private Path scorePreload, park;
    ...
    public void buildPaths(){
        ...
        intakeSpecimen1 = follower.pathBuilder()
                ...
                .setLinearHeadingInterpolation(redSpecimen_scorePreloadPose.getHeading(), redSpecimen_spikeLeftPose.getHeading(), 0.6)
                .build();
        ...
        park = follower.pathBuilder()
                .addPath(new BezierLine(new Point(redSpecimen_spikeRightPose), new Point(redSpecimen_parkPose)))
                .setLinearHeadingInterpolation(redSpecimen_spikeRightPose.getHeading(), redSpecimen_parkPose.getHeading())
                .build();
    }
        ...
        switch (pathState){
            ...
            case 2:
                ...
                if (Math.abs(follower.getPose().getX() - redSpecimen_spikeLeftPose.getX()) < positionThreshold && Math.abs(follower.getPose().getY() - redSpecimen_spikeLeftPose.getY()) < positionThreshold){
                    ...
                    if (pathTimer.getElapsedTime() - pathTimeStamp > 2300){
                        ...
                        follower.followPath(intakeSpecimen2, true);
                        setPathState(3);
                        debounce = true;
                        timerActive = false;
                    }
            ...
            case 3:
                ...
                if (Math.abs(follower.getPose().getX() - redSpecimen_spikeMiddlePose.getX()) < positionThreshold && Math.abs(follower.getPose().getY() - redSpecimen_spikeMiddlePose.getY()) < positionThreshold){
                    ...
                    if (pathTimer.getElapsedTime() - pathTimeStamp > 2300){
                        ...
                        // hold endpoint while scoring sample
                        follower.followPath(intakeSpecimen3, true);
                        setPathState(4);
                        debounce = true;
                        timerActive = false;
                    }
            ...
            case 4:
                if (Math.abs(follower.getPose().getX() - redSpecimen_spikeRightPose.getX()) < positionThreshold && Math.abs(follower.getPose().getY() - redSpecimen_spikeRightPose.getY()) < positionThreshold){
                    ...
                    if (actionTimer.getElapsedTime() >= 600 && actionTimer.getElapsedTime() < 800){
                        setIntake(0.6, intakeFlexorGrab, intakeElbowGrab);
                    }
                    if (actionTimer.getElapsedTime() >= 800 && actionTimer.getElapsedTime() < 1100){
                        intakeGrabber.setPosition(intakeGrabberClosed);
                    }
                ...
                break;
            case 5:
                if (Math.abs(follower.getPose().getX() - redSpecimen_parkPose.getX()) < positionThreshold && Math.abs(follower.getPose().getY() - redSpecimen_parkPose.getY()) < positionThreshold){
                    intakeGrabber.setPosition(intakeGrabberOpen);
                    // start timer
                    if (!timerActive){
                        pathTimeStamp = pathTimer.getElapsedTime();
